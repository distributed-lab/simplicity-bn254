...
⚔️ __The size of compiled program which evaluates one pairing is 10.5 KBytes and it consumes ~4.2M weight units.__
...
### Building program
Check [build.sh](./scripts/build.sh) bash script for additional information. It currently configured to build and run [example-main.simf](./example-main.simf) file using [example-witness.wit](./example-main.simf) witness which is the same as in `test_pair()`.
